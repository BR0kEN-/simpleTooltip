...
;(function($){$.STHP=function(e,w){var offset=e.offset().left,right=$(window).width()-(offset+e.width()+20)<w,left=offset-20<w,val=e.data('hint');if(left||right){if(val.length!=5){e.attr('data-hint',val.length==2?val[0]+(left?'w':right?'e':''):(left?'west':right?'east':''))}}else e.attr('data-hint',val)}})(jQuery);